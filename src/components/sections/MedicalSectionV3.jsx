...
const MedicalSectionV3 = ({ inView, sectionRef }) => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      setIsTabletLayout(window.innerWidth < 1200);
    };
  ...
  const gantryFrameStyle = {
    ...
    transform: shouldTransition
      ...
      : videoOffscreenTransform, // Always reset position when not transitioning
  ...
  useLayoutEffect(() => {
    ...
    if (isTabletLayout) {
      const baseTop = 60;
      setCollectionTop(`${baseTop}px`);
      setVideoAndCaptionTop(`${baseTop + headerHeight + gap}px`);
      return;
    }
  ...
  }, [headerHeight, gap, videoHeight, isTabletLayout]);

  // Responsive breakpoint already handled by isTabletLayout above
    ...
    >
      ...
      <div style={{
        ...
        <div
          ...
          style={{
            position: isTabletLayout ? 'relative' : 'absolute',
            left: isTabletLayout ? 'auto' : 'calc(50% + 20px)', // CHANGED: was right
        ...
        >
          ...
          {shouldTransition && (
            <div style={{
              position: 'absolute',
              left: isTabletLayout ? '50%' : 0, // CHANGED: was right: 0
              top: 0,
          ...
          {shouldTransition && (
            <div
              ...
              style={{
                ...gantryFrameStyle,
                position: isTabletLayout ? 'relative' : 'absolute',
                left: isTabletLayout ? 'auto' : 0, // CHANGED: was right: 0
                top: isTabletLayout ? 'auto' : 0,
                zIndex: 3,
                pointerEvents: 'auto',
        ...
        >
          ...
          >
            <div
              className="relative flex flex-col gap-2 items-start justify-start"
              style={{
                width: '100%',
                marginRight: 0,
                paddingRight: 0,
      ...
      <div
        ...
        style={{
          ...
          top: isTabletLayout ? 'auto' : collectionTop,
          width: isTabletLayout ? 'min(520px, 90vw)' : 480,
          background: 'none',
          zIndex: 20,
          transition: shouldTransition ? (headerVisible ? 'opacity 2.25s ease' : 'none') : 'none',
          opacity: headerVisible ? 1 : 0,
          margin: isTabletLayout ? '0 auto 32px' : 0,
        }}
      >
        <div style={{
          width: '100%',
          ...
          <h2 style={{
            ...
            alignSelf: isTabletLayout ? 'center' : 'flex-end',
            paddingRight: 0,
            textAlign: isTabletLayout ? 'center' : 'right',
            width: '100%',
            userSelect: 'none',
            WebkitUserSelect: 'none'
          }}>
