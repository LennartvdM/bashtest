      ...
      <div className={`flex flex-col md:flex-row gap-10 grow items-center md:items-start ${reverse ? "md:flex-row-reverse" : ""}`}>
        ...
        <div className="relative basis-1/2 overflow-hidden rounded-2xl bg-gray-300 min-h-[260px] min-w-[340px] max-w-[420px] w-full h-[240px] md:h-[280px]">
          {slides.map((s, i) => (
            <div
              key={s.id}
              className={`absolute inset-0 flex items-center justify-center text-6xl md:text-7xl text-teal-600 font-bold transition-opacity duration-1000 ease ${
        ...
        >
          ...
          {ready && Number.isFinite(current) && (
            <div
              className="absolute left-0 w-full rounded-xl bg-white/90 shadow-md transition-all duration-1000 ease pointer-events-none"
          ...
          {headlines.map((text, i) => (
            ...
            >
              <p
                className={`text-lg md:text-xl font-medium transition-all duration-1000 ease ${
                  current === i
