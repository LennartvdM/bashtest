...
const MedicalSectionV3 = ({ inView, sectionRef }) => {
  ...
  const [highlighterLeft, setHighlighterLeft] = useState({ x: 0, y: 0 }); // RENAMED FROM highlighterRight
  ...
  // The video container is left: 'calc(50% + 20px)', so its right edge is at window.innerWidth/2 + 20px + 480px
  // The band should be positioned so its cutout's left edge matches the video container's right edge
  ...
  const gantryFrameStyle = {
    ...
    transform: safeVideoHover 
      ...
      : videoVisible 
        ? 'translateX(0)' 
        : 'translateX(200px)', // CHANGED: was -200px
  ...
  // Duplicated handleBarEnd for left highlighter
  const handleBarEnd = () => {
    ...
    <div ref={sectionRef} className="h-screen w-full relative overflow-hidden bg-[#f5f8fa]">
      ...
      <div style={{
        ...
        <div
          ...
          style={{
            position: 'absolute',
            right: 'calc(50% + 20px)', // CHANGED: was left
            top: videoAndCaptionTop,
            width: 444,
            height: videoHeight,
            display: 'flex',
            ...
            transform: captionsVisible ? 'translateX(0)' : 'translateX(-200px)', // CHANGED: was 200px
          }}
        >
      ...
      >
        <div style={{ width: 480, display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-start', marginLeft: 0 }}> {/* CHANGED: was marginRight */}
          <h2 style={{
            ...
            Yet, <span style={{ color: '#3fd1c7' }}>reflection</span><br />
            strengthens<br />
            the next
          </h2>
        </div>
