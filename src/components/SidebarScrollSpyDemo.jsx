// SidebarScrollSpyDemo.jsx
// React 18 · Tailwind CSS 3 · framer-motion 10 (plain JS)
// Smooth-scroll article with animated scroll-spy sidebar.
// Preface un-numbered – subsequent chapters auto-numbered.
// Sidebar highlight locks perfectly to the section in view.

import React, { useLayoutEffect, useState } from 'react';
import { motion, Variants } from 'framer-motion';

/**************** 0 · Optional global helper ****************/
// In your CSS:  html { @apply scroll-smooth; }
...
function useScrollSpy(ids) {
  ...
  useLayoutEffect(() => {
    ...
    const TRIGGER_Y = 120; // px from top where section becomes active
    let ticking = false;
  ...
  // immediate update on sidebar click
  useLayoutEffect(() => {
...
/**************** 3 · Indicator variants (uniform height) ****************/
...
export default function SidebarScrollSpyDemo() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0d0c44]/40 to-[#1488cc]/40">
      {/* top bar */}
      <header className="sticky top-0 z-30 flex h-12 items-center justify-center gap-8 rounded-b-md bg-white/70 backdrop-blur-md">
      ...
      <main className="mx-auto grid max-w-6xl grid-cols-[260px_1fr] gap-8 px-4 pb-24 md:pt-16">
        ...
        {/* article */}
        <article className="space-y-24 rounded-lg bg-white/[.85] px-10 py-14 shadow-lg backdrop-blur-md" itemScope itemType="https://schema.org/Article">
          {SECTIONS.map((s, idx) => (
            <section key={s.id} id={s.id} className="scroll-mt-28">
              <h2 className="mb-6 text-3xl font-extrabold text-[#0d0c44]">{idx === 0 ? s.raw : \`\${idx}. \${s.raw}\`}</h2>
...
/**************** 7 · Tailwind helper ****************/
// If you want the brand color utility, add to tailwind.config.js:  theme.extend.colors.brand = '#0d0c44';
